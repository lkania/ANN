function [training_set test_set] = get_trainning_set(path,activate_normalization)
	...
	if(activate_normalization)
		information_set(:,1) = ((information_set(:,1)-min(information_set(:,1)))./(max(information_set(:,1))-min(information_set(:,1))));
		information_set(:,2) = ((information_set(:,2)-min(information_set(:,2)))./(max(information_set(:,2))-min(information_set(:,2))));
		information_set(:,3) = ((information_set(:,3)-min(information_set(:,3)))./(max(information_set(:,3))-min(information_set(:,3))));
	end

	patterns_quantity = size(information_set)(1);

	training_set_size = ceil(patterns_quantity*2/3);
